...
We divide the training data in a train partition and a validation partition and define the trainControl method to be "oob" (out of bag).
...
The confusion matrix on the training set is an estimate of the out of sample error.
...
As always, this estimate is optimistic since the model has been constructed on this same data set.
...
We could have also used them in an "ensemble" implementation but the accuracy of the random forest algorithm was enough in this case.
...
We now apply the model to the validation data set to validate the out of sample estimate:
...
Finally, we apply this model to the testing set to make our predictions ...
